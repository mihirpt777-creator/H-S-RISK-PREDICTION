...
/* Force accent color on native widgets directly, so theming doesn't depend on .streamlit/config.toml */
[data-testid="stRadio"] label div:first-child > div {
    border-color: #D98E4A !important;
}
[data-testid="stRadio"] label div:first-child > div > div {
    background-color: #D98E4A !important;
}
button[data-baseweb="tab"][aria-selected="true"] {
    color: #D98E4A !important;
    border-bottom-color: #D98E4A !important;
}
.stSlider [role="slider"] { background-color: #D98E4A !important; }
.stButton button, .stDownloadButton button {
    border-color: #D98E4A !important;
    color: #D98E4A !important;
}
.stButton button:hover { background-color: #D98E4A22 !important; }
</style>
...
def find_smartctl():
    """Find smartctl reliably, without depending on this session's PATH being up to date."""
    import shutil
    ...
    for p in common_paths:
        if _os.path.exists(p):
            return p
    return "smartctl"  # fall back to bare command, will raise FileNotFoundError with a clear message


def find_powercfg():
    """Find powercfg reliably, same reasoning as find_smartctl above."""
    import shutil
    found = shutil.which("powercfg")
    if found:
        return found
    common_paths = [
...
if domain.startswith("💾"):
    ...
    with tab1:
        ...
        if st.button("Scan now"):
            try:
                scan = subprocess.run([find_smartctl(), "--scan"], capture_output=True, text=True)
                ...
                if not lines:
                    ...
                    result = subprocess.run([find_smartctl(), "-a", "-j", device], capture_output=True, text=True)
    ...
    with tab1:
        ...
        if st.button("Read my real battery health"):
            try:
                ...
                subprocess.run([find_powercfg(), "/batteryreport", "/xml", "/output", tmp_path],
                                capture_output=True, text=True, timeout=20)
                tree = ET.parse(tmp_path)

                # Search by local tag name only, ignoring namespace -- robust across Windows versions
                design_cap, full_cap, cycle_count = None, None, 0
                for elem in tree.iter():
                    local_tag = elem.tag.split("}")[-1]
                    if local_tag == "DesignCapacity" and design_cap is None and elem.text and elem.text.strip():
                        design_cap = int(elem.text)
                    elif local_tag == "FullChargeCapacity" and full_cap is None and elem.text and elem.text.strip():
                        full_cap = int(elem.text)
                    elif local_tag == "CycleCount" and elem.text:
                        cycle_count = int(elem.text)

                if design_cap is None or full_cap is None:
                    all_tags = sorted(set(elem.tag.split("}")[-1] for elem in tree.iter()))
                    st.warning("Couldn't find capacity data in the report on this system.")
                    st.caption(f"Diagnostic — tags actually found in the report: {', '.join(all_tags[:25])}")
                else:
                    ...
                    if cycle_count == 0 and health_pct < 90:
                        st.warning(
                            f"This estimate likely isn't reliable: your real health ({health_pct:.1f}%) shows "
                            "real wear, but Windows didn't report a real cycle count here, so the model saw "
                            "'0 cycles' and assumed a nearly-new battery. **The health percentage above is "
                            "the trustworthy real number** — this model estimate is a known limitation of "
                            "partial data, not a contradiction."
                        )
            except FileNotFoundError:
    ...
    st.info(
        "Trained on 150,000 real SSD SMART records from Alibaba's data centers. The matching "
        "failure-label file wasn't reachable, so this uses **anomaly detection** — flagging drives "
        "whose readings look statistically unlike the rest of the real fleet — a real, published "
        "technique for this exact situation."
    )

    tab1, tab2 = st.tabs(["🔎 Scan this computer's real drive (SSD-matched)", "🎚️ Try your own values"])

    with tab1:
        st.write(
            "This uses the SAME standard SMART attribute numbers your SSD actually reports — "
            "unlike the Hardware tab (trained on mechanical drives), this model was trained on real "
            "SSDs, so it should give a genuinely trustworthy result for your drive."
        )
        if st.button("Scan now (SSD-matched)"):
            try:
                scan = subprocess.run([find_smartctl(), "--scan"], capture_output=True, text=True)
                lines = [l for l in scan.stdout.splitlines() if l.strip() and not l.startswith("#")]
                if not lines:
                    ...
                    table = data.get("ata_smart_attributes", {}).get("table", [])
                    attr_by_id = {a["id"]: a.get("value", 0) for a in table}  # normalized value, matches n_* columns

                    found = sum(1 for f in feature_names if int(f.split("_")[1]) in attr_by_id)
                    coverage = found / len(feature_names)
                    st.write(f"Real attribute coverage: {found}/{len(feature_names)} ({coverage:.0%})")

                    x = np.array([attr_by_id.get(int(f.split("_")[1]), ssd_saved["ranges"][f][2]) for f in feature_names])
                    score = model.decision_function([x])[0]
                    pct = (score - ssd_saved["score_min"]) / (ssd_saved["score_max"] - ssd_saved["score_min"]) * 100
                    pct = max(0, min(100, pct))

                    if coverage < 0.5:
                        st.warning("Coverage still low for this drive — result below may not be fully reliable, but should be far better matched than the Hardware tab.")
                    st.metric("Normality score", f"{pct:.1f}/100", help="Higher = looks more like a typical healthy real SSD")
                    st.error("⚠️ Anomalous — unlike 95% of the real fleet") if score < ssd_saved["score_p5"] else st.success("✅ Looks normal")
            except FileNotFoundError:
                st.error("smartctl not found. Install smartmontools first.")
            except Exception as e:
                st.error(f"Could not read drive data: {e}")
    ...
    with tab2:
        ...
        for i, fname in enumerate(feature_names):
            ...
            with cols[i % 2]:
                if mn == mx:
                    st.caption(f"{fname}: always {mn:g} in this real dataset (no variation to adjust)")
                    vals[fname] = mn
                else:
